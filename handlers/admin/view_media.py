...
async def view_music(callback: CallbackQuery):
    ...
    music = [
        r for r in rows
        if r.get("work_content").split(":")[0] == "audio" and r.get("work_content")
    ]

    if not music:
        await callback.message.answer("Немає музики 😔")
        await callback.answer()
        return
    ...
    try:
        for row in music:
            ...
            # Логуємо, щоб бачити, що відправляємо (для відладки)
            logger.info(f"Attempt to send music with ID: '{file_id}'")
            ...
            try:
                ...
                logger.warning(f"Failed to send audio {file_id}: {e}")
                await callback.message.answer(f"Помилка з фото автора {row.get('author_name')}")
